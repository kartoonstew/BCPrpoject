...
Each weapon on the combat page has editable **Custom hit** and **Custom damage** inputs and a **Use custom** checkbox. Tick it to replace either final total independently; a blank custom input retains the configured result. Zero is a valid hit override. Unticking restores automatic calculations unless the weapon is in Manual mode or the global House rules switch is On. Manual mode always uses these inputs, and the global switch continues to activate existing overrides for compatibility. Custom entries include all bonuses and penalties. The matching per-weapon switch is also available in the website configuration and JSON backups. Advanced weapon properties remain on the Weapon configuration page. Calculated results remain read-only; the custom fields stay editable even in readers without scripting, though those readers cannot update the calculated results. In scripting-capable readers they recalculate immediately. Ability descriptions, style reference text and continuation notes are export snapshots: re-export to refresh them after changes.
