...
## Equipment and house-rule revision — 2026-09-06

- 24 Node tests pass. New coverage checks every armor entry, negative Dexterity, armor training versus mithral, Stealth roll state versus flat skill bonus, 2024 exhaustion levels 0–6, finesse/thrown/Heavy/versatile weapons, Fighting Styles, campaign equipment features, and overrides including zero and dormant values.
- Chrome desktop/mobile workflow checks pass with weapon preset selection, grip changes, automatic AC, override On/Off, persisted drafts, JSON import/export, and filled/blank PDF downloads. No page errors or mobile horizontal overflow.
- The actual exported script runs in PDF.js QuickJS with committed equipment dropdown changes, exhaustion, custom AC/speed/Stealth replacements and weapon calculation edits, in addition to earlier ability/level/checkbox regressions.
- All 36 subclass/milestone export combinations retain exactly the unlocked editable features. Canonical fields agree with widgets and have appearance streams. The blank now has 6 pages / 231 fields; Rook has 8 pages / 242 fields. Calculation order includes 59 fields in the blank and 58 at level 7. Random wear changes without altering character values.
- The two added configuration pages and revised combat/training pages were rendered and visually inspected. Apple PDFKit renders every final sample page and preserves an edited description through save/reopen. LibreOffice headless import/export succeeds and retains visible saved equipment values. This is not a physical iPad or direct Acrobat UI certification.
- Static build includes the new shared conditional-rules module. Module URLs use `conditions1` to refresh previously cached imports. Source scope, manual modes and viewer limitations are documented in `equipment-rules.md`.
