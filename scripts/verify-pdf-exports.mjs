...
for(const c of db.classes)for(const level of [1,3,7,10])for(const orientation of ['portrait','landscape']){
 ...
 for(const key of ['penisSize','ballSize']){const f=pdf.getForm().getTextField(key);assert(!f.isReadOnly());assert.equal(f.acroField.getWidgets()[0].P().toString(),pdf.getPages()[0].ref.toString());}
