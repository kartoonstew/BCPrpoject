...
try{
 ...
 await page.locator('#level').fill('5');await page.locator('#strength').fill('18');
 for(const orientation of ['portrait','landscape']){
  ...
  const result=await page.evaluate(async bytes=>{const pdf=await PDFLib.PDFDocument.load(new Uint8Array(bytes)),form=pdf.getForm();return {sizes:pdf.getPages().map(p=>p.getSize()),penis:form.getTextField('penisSize').getText(),balls:form.getTextField('ballSize').getText(),str:form.getTextField('strength_mod').getText()};},[...await readFile(file)]);
  assert(result.sizes.every(s=>s.width===(orientation==='landscape'?792:612)&&s.height===(orientation==='landscape'?612:792)));
  assert.equal(result.penis,'37');assert.equal(result.balls,'64');assert.equal(result.str,'+4');
  assert.equal(d.suggestedFilename().includes('-landscape-'),orientation==='landscape');
 }
 await page.reload();assert.equal(await page.locator('#penisSize').inputValue(),'37');assert.equal(await page.locator('#ballSize').inputValue(),'64');
 ...
 assert.deepEqual(errors,[]);console.log('Browser passed: both orientations, editable percentage values, saved draft, blank landscape and responsive download selector.');
