import {campaignBriefing,recapText} from './campaign-recaps.js?v=recaps1';
import {buildScenes,groupSceneRecords} from './campaign-scenes.js?v=recaps1';
...
function renderCampaignBriefing(entries,scenes){
 ...
 return `<details id="campaign-briefing" class="campaign-briefing"><summary><span><small class="briefing-kicker">Catch up before the next session</small><strong>Campaign to date</strong><span class="briefing-through">Full archive · Through ${date(b.through+'T00:00:00Z')}</span></span><span class="recap-duration">${minutes} min briefing <b aria-hidden="true"></b></span></summary><div class="briefing-body"><p class="recap-editorial">Editorial synthesis of the writings and conversations · Covers the full archive, regardless of filters</p><div class="briefing-current"><h2>Where we stand</h2><p>${escape(b.current)}</p></div><ol class="briefing-beats">${b.beats.map(beat=>`<li><h3>${escape(beat.title)}</h3><p>${escape(beat.text)}</p>${recapSources(beat.sources||[],beat.scenes||[],entries,scenes)}</li>`).join('')}</ol><section class="briefing-threads"><h2>Still unresolved</h2><ul>${b.threads.map(t=>`<li>${escape(t)}</li>`).join('')}</ul></section><div class="recap-actions"><span>Latest available records · ${date(b.through+'T00:00:00Z')}</span><button type="button" data-close-recap>Close briefing ↑</button></div></div></details>`;
}
