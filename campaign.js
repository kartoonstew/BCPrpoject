import {campaignBriefing,recapText,summaryPreviews} from './campaign-recaps.js?v=summaries1';
import {buildScenes,groupSceneRecords} from './campaign-scenes.js?v=summaries1';
...
const summaryPreviewText=e=>e.summary&&summaryPreviews[e.id]?recapText(summaryPreviews[e.id]):'';
const plain=e=>[e.title,e.description,e.period,e.author,e.voice,e.body,summaryPreviewText(e),...(e.blocks||[]).map(b=>b.text)].filter(Boolean).join(' ');
const isSummary=e=>e.kind==='document'&&e.summary===true;
const byTime=(a,b)=>new Date(a.published)-new Date(b.published)||a.id.localeCompare(b.id);
const bySession=(a,b)=>(a.summaryOrder??Infinity)-(b.summaryOrder??Infinity)||byTime(a,b);
/** Session summaries read in session order; every other record type reads in posting order. */
export const orderRecords=(records,kind='all')=>records.sort(kind==='summaries'?bySession:byTime);
export function filterCampaign(entries,{kind='all',q='',author='',month=''}={}){
 ...
 return orderRecords(entries.filter(e=>(kind==='all'||(kind==='documents'?e.kind!=='ic':kind==='summaries'?isSummary(e):e.kind==='ic'))&&(!author||e.author===author)&&(!month||e.published.startsWith(month))&&terms.every(t=>plain(e).toLocaleLowerCase().includes(t))),kind);
}
...
export async function renderCampaign({main,path,params}){
 ...
 try{cached ||= fetch('data/campaign.json?v=summaries1').then(r=>{if(!r.ok)throw Error('Archive unavailable');return r.json();});const db=await cached;
...
function renderTimeline(main,db,params){
 const entries=db.entries,scenes=buildScenes(entries),docs=entries.filter(e=>e.kind==='document'),summaries=entries.filter(isSummary),latest=docs.at(-1),authors=[...new Set(entries.filter(e=>e.kind==='ic').map(e=>e.author))].sort(),months=[...new Set(entries.map(e=>e.published.slice(0,7)))];
 const latestLabel=latest.dateKind==='added'?`Added to the archive / ${date(latest.published)}`:`Latest writing / ${date(latest.published)}`;
 const initial={kind:params.get('kind')||'all',q:params.get('q')||'',author:params.get('author')||'',month:params.get('month')||''};
 main.innerHTML=`<div class="campaign-wrap"><header class="campaign-intro"><div><div class="eyebrow">The Annals / Campaign records</div><h1>The Company<br><em>remembers.</em></h1><p>The written record and the voices between the lines.<br>Orders, accounts, and words passed among Brothers.</p><div class="archive-tally"><span><b>${docs.length}</b> writings</span><span><b>${summaries.length}</b> session summaries</span><span><b>${entries.filter(e=>e.kind==='ic').length}</b> posts</span><span><b>${authors.length}</b> contributors</span></div></div><a class="latest-writing" href="${docLink(latest.id)}"><span class="eyebrow">${latestLabel}</span><h2>${escape(latest.title)}</h2><p>${escape(latest.description)}</p><span class="text-link">Open the record ↗</span></a></header>
 ${renderCampaignBriefing(entries,scenes)}
 ...
 <section class="archive-controls" aria-label="Filter the campaign archive"><div class="archive-tabs" role="group" aria-label="Record type">${[['all','Everything'],['summaries','Session summaries'],['documents','Writings & maps'],['ic','In-character posts']].map(([v,l])=>`<button type="button" data-kind="${v}" aria-pressed="${v===initial.kind}">${l}</button>`).join('')}</div><div class="archive-search-row"><label class="archive-search"><span>Search the archive</span><input id="archive-search" type="search" value="${escape(initial.q)}" placeholder="A Brother, a place, a remembered phrase…"></label><label><span>Contributor</span><select id="archive-author"><option value="">Everyone</option>${authors.map(a=>`<option ${a===initial.author?'selected':''}>${escape(a)}</option>`).join('')}</select></label><label><span>Filed / added in</span><select id="archive-month"><option value="">Every month</option>${months.map(m=>`<option value="${m}" ${m===initial.month?'selected':''}>${date(m+'-01T00:00:00Z',{day:undefined,month:'long'})}</option>`).join('')}</select></label></div></section>
 <div class="archive-meta"><div><p id="archive-count" role="status"></p><label class="table-talk-control" id="feed-talk-control"><input type="checkbox" id="feed-talk" checked> Show table talk</label></div><p><span id="archive-meta-note">Oldest first · Record dates in UTC</span><br><span id="archive-date-note"></span></p></div>
 ...
 bindRecapControls(main.querySelector('#campaign-briefing'));
 ...
 function draw(){
  const opened=[...main.querySelectorAll('.archive-entry details[open]')].map(d=>({record:d.closest('.archive-entry').id,type:d.className}));
  ...
  const filtered=view==='feed'?matches.filter(e=>feedTalk.checked||!e.aside):[
   ...
   ...(state.kind==='documents'?[]:scenes.filter(scene=>scene.posts.some(p=>matchedIds.has(p.id))||(
    state.q.trim()&&state.q.toLowerCase().trim().split(/\s+/).every(t=>(scene.title+' '+scene.summary+' '+recapText(scene.recap)).toLowerCase().includes(t))&&filterCampaign(scene.posts,{...state,q:''}).length
   )))
  ];
  orderRecords(filtered,state.kind);
  ...
  main.querySelector('#archive-meta-note').textContent=state.kind==='summaries'?'Session order · Record dates in UTC':'Oldest first · Record dates in UTC';
  ...
  main.querySelector('#campaign-stream').innerHTML=(shown.length?shown.map(e=>{const d=e.published.slice(0,view==='scenes'?7:10),heading=d!==day?`<h2 class="archive-day"><time datetime="${d}">${e.dateKind==='added'?'Added to the archive · ':''}${date(e.published,view==='scenes'?{day:undefined,month:'long'}:{})}</time></h2>`:'';day=d;return heading+(e.kind==='scene'?renderSceneCard(e,state,matches,entries):state.kind==='summaries'&&isSummary(e)?renderSummaryCard(e,state,entries):renderEntry(e,entries,state.q));}).join(''):'<div class="archive-empty"><h2>No records match.</h2><p>Try another name, phrase, or date.</p><button class="button" data-clear>Clear filters</button></div>')+(shown.length<filtered.length?`<button class="button archive-more" data-more>Read the next ${Math.min(40,filtered.length-shown.length)} records <span>↓</span></button>`:'<p class="archive-end">— End of this record —</p>');
  for(const {record,type} of opened){const details=[...document.getElementById(record)?.querySelectorAll('details')||[]].find(d=>d.className===type);details?.setAttribute('open','');}
  bindRecapControls(main.querySelector('#campaign-stream'));
 ...
 function update(){if(!main.querySelector('#campaign-stream'))return;target=null;limit=40;const query=Object.fromEntries(Object.entries(state).filter(([k,v])=>v&&(k!=='kind'||v!=='all')));if(view==='feed')query.view='feed';if(!feedTalk.checked)query.talk='hide';history.replaceState(null,'',link(query));draw();}
...
const readingMinutes=text=>Math.max(1,Math.ceil(text.split(/\s+/).filter(Boolean).length/220));
function recapSources(sourceIds,sceneIds,entries,scenes){
 const sources=sourceIds.map(id=>entries.find(e=>e.id===id)).filter(Boolean),related=sceneIds.map(id=>scenes.find(s=>s.id===id)).filter(Boolean);
 return `<div class="recap-sources"><span>From the record</span>${sources.map(e=>`<a href="${docLink(e.id)}">${escape(e.title)} ↗</a>`).join('')}${related.map(s=>`<a href="${sceneLink(s.id,{recap:'1'})}">${escape(s.title)} ↗</a>`).join('')}</div>`;
}
function renderSceneRecap(scene,entries,{open=false}={}){
 const r=scene.recap;if(!r)return '';
 return `<details class="scene-recap" ${open?'open':''}><summary><span><strong>Scene recap</strong><small>Context, key events & outcome</small></span><span class="recap-duration">${readingMinutes(recapText(r))} min read <b aria-hidden="true"></b></span></summary><div class="recap-body"><p class="recap-editorial">Editorial recap · Based on the original record</p><h4>The situation</h4><p>${escape(r.context)}</p><h4>What happened</h4><ul>${r.events.map(e=>`<li>${escape(e)}</li>`).join('')}</ul><div class="recap-outcome"><h4>Where it leaves us</h4><p>${escape(r.outcome)}</p></div>${r.sources.length?recapSources(r.sources,[],entries,[]):''}<div class="recap-actions"><a href="${sceneLink(scene.id,{entry:scene.first})}">Read original conversation →</a><button type="button" data-close-recap>Close recap ↑</button></div></div></details>`;
}
function renderSummaryPreview(record,entries,{open=false}={}){
 const r=summaryPreviews[record.id];if(!r)return '';
 return `<details class="scene-recap summary-preview" ${open?'open':''}><summary><span><strong>Summary preview</strong><small>${escape(record.label)} · context, key events & outcome</small></span><span class="recap-duration">${readingMinutes(recapText(r))} min preview <b aria-hidden="true"></b></span></summary><div class="recap-body"><p class="recap-editorial">Editorial preview · Grounded in the supplied account</p><h4>The situation</h4><p>${escape(r.context)}</p><h4>What happened</h4><ul>${r.events.map(e=>`<li>${escape(e)}</li>`).join('')}</ul><div class="recap-outcome"><h4>Where it leaves us</h4><p>${escape(r.outcome)}</p></div>${r.sources.length?recapSources(r.sources,[],entries,[]):''}<div class="recap-actions"><a href="${docLink(record.id)}">Read the full account →</a><button type="button" data-close-recap>Close preview ↑</button></div></div></details>`;
}
function renderSummaryCard(e,state,entries){
 const preview=summaryPreviews[e.id],needle=state.q.toLocaleLowerCase().trim();
 const open=Boolean(needle)&&preview&&needle.split(/\s+/).every(t=>recapText(preview).toLowerCase().includes(t));
 return `<article id="record-${e.id}" class="archive-entry annals-entry summary-card"><div class="annals-entry-top"><span>${escape(e.label)}</span>${stamp(e)}</div><h3><a href="${docLink(e.id)}">${escape(e.title)}</a></h3><p class="story-period">${escape(e.period)}</p><p class="summary-context">${escape(preview?preview.context:e.description)}</p>${renderSummaryPreview(e,entries,{open})}<div class="annals-entry-bottom"><span>Session ${e.summaryOrder} · Full account ${Math.ceil(e.words/220)} min read${e.dateKind==='added'?' · Added to the archive':''}</span><a href="${docLink(e.id)}">Read the full account ↗</a></div></article>`;
}
const briefingNote=()=>campaignBriefing.throughNote?`<p class="briefing-note">${escape(campaignBriefing.throughNote)}</p>`:'';
function renderCampaignBriefing(entries,scenes){
 const b=campaignBriefing,minutes=readingMinutes([b.current,...b.beats.map(x=>x.title+' '+x.text),...b.threads].join(' '));
 return `<details id="campaign-briefing" class="campaign-briefing"><summary><span><small class="briefing-kicker">Catch up before the next session</small><strong>Campaign to date</strong><span class="briefing-through">Full archive · Through ${date(b.through+'T00:00:00Z')}</span></span><span class="recap-duration">${minutes} min briefing <b aria-hidden="true"></b></span></summary><div class="briefing-body"><p class="recap-editorial">Editorial synthesis of the writings and conversations · Covers the full archive, regardless of filters</p><div class="briefing-current"><h2>Where we stand</h2><p>${escape(b.current)}</p>${briefingNote()}</div><ol class="briefing-beats">${b.beats.map(beat=>`<li><h3>${escape(beat.title)}</h3><p>${escape(beat.text)}</p>${recapSources(beat.sources||[],beat.scenes||[],entries,scenes)}</li>`).join('')}</ol><section class="briefing-threads"><h2>Still unresolved</h2><ul>${b.threads.map(t=>`<li>${escape(t)}</li>`).join('')}</ul></section><div class="recap-actions"><span>Latest available records · ${date(b.through+'T00:00:00Z')}</span><button type="button" data-close-recap>Close briefing ↑</button></div></div></details>`;
}
function bindRecapControls(root){
 root?.querySelectorAll('[data-close-recap]').forEach(button=>button.addEventListener('click',()=>{const details=button.closest('details');details.open=false;details.querySelector('summary').focus({preventScroll:true});details.scrollIntoView({block:'nearest'});}));
}
function renderSceneCard(scene,state,matches,entries){
 const ids=new Set(scene.posts.map(p=>p.id)),hits=state.q.trim()?matches.filter(p=>ids.has(p.id)):[];
 return `<article class="archive-entry scene-card" id="record-${scene.id}"><div class="scene-card-meta"><span>Scene ${String(scene.number).padStart(2,'0')}</span><span>${date(scene.published,{year:undefined})} – ${date(scene.ended)}</span></div><h3><a href="${sceneLink(scene.id)}">${escape(scene.title)} <span aria-hidden="true">↗</span></a></h3><p class="scene-summary"><small>Editorial summary</small>${escape(scene.summary)}</p>${renderSceneRecap(scene,entries,{open:!!state.q.trim()&&state.q.toLowerCase().trim().split(/\s+/).every(t=>recapText(scene.recap).toLowerCase().includes(t))})}<div class="scene-participants">${scene.voices.map(escape).join(' · ')}</div><footer><span>${scene.posts.length} posts · ${Math.ceil(scene.words/220)} min read${scene.posts.some(p=>p.aside)?' · Includes table talk':''}</span><a href="${sceneLink(scene.id)}">Read conversation →</a></footer>${hits.length?`<details class="scene-matches" open><summary>${hits.length} matching ${hits.length===1?'post':'posts'} — open in context</summary><div>${hits.map(p=>`<a href="${sceneLink(scene.id,{entry:p.id,q:state.q})}"><strong>${escape(p.voice)}${p.aside?' · Table talk':''}</strong><time datetime="${p.published}">${date(p.published,{year:undefined})}</time><span>${escape(excerpt(p,state.q))}</span></a>`).join('')}</div></details>`:''}</article>`;
}
...
function stamp(e){
 if(e.dateKind==='added')return `<a class="entry-time" href="${link({entry:e.id})}" aria-label="Link to record added to the archive ${date(e.published)}"><time datetime="${escape(e.published)}">Added ${date(e.published)}</time> ↗</a>`;
 return `<a class="entry-time" href="${link({entry:e.id})}" aria-label="Link to record posted ${date(e.published)} at ${clock(e.published)} UTC"><time datetime="${escape(e.published)}">${clock(e.published)}</time> ↗</a>`;
}
const sourceDownloadLabel=e=>e.originalDownload?'Current source document ↓':'Original document ↓';
const originalDownloadLink=e=>e.originalDownload?`<a href="${escape(e.originalDownload)}" download>Original filing DOCX ↓</a>`:'';
const railProvenance=e=>e.updatedAt?`<p class="rail-provenance">Original filing ${date(e.published)} · expanded source added ${date(e.updatedAt+'T00:00:00Z')}</p>`:'';
function renderEntry(e,entries,q){
 if(e.kind==='ic'){
  ...
  return `<article id="record-${e.id}" class="archive-entry dispatch ${e.author==='smalls'?'dispatch-narrator':''} ${e.aside?'dispatch-aside':''}"><div class="dispatch-seal" aria-hidden="true">${escape(e.voice==='Narrator & NPCs'?'BC':e.voice.slice(0,2).toUpperCase())}</div><div class="dispatch-content"><header><div><strong>${escape(e.voice)}</strong><span>${escape(e.author)}${e.aside?' · Table talk':e.author==='smalls'?' · Narration':' · In character'}${e.edited?' · Edited':''}</span></div>${stamp(e)}</header>${replyHtml}${folded?`<details class="dispatch-long" ${q&&!e.aside?'open':''}><summary><span>${escape(excerpt(e,q))}</span><b>${e.aside?'Read table aside':'Read full post'} ↓</b></summary>${content}</details>`:content}</div></article>`;
 }
 return `<article id="record-${e.id}" class="archive-entry annals-entry"><div class="annals-entry-top"><span>${escape(e.label)}</span>${stamp(e)}</div><h3><a href="${e.kind==='document'?docLink(e.id):escape(e.download)}" ${e.kind==='attachment'?'target="_blank" rel="noopener"':''}>${escape(e.title)}</a></h3><p class="story-period">${escape(e.period)}</p><p>${escape(q?excerpt(e,q):e.description)}</p>${e.image?`<a class="archive-attachment" href="${escape(e.download)}" target="_blank" rel="noopener"><img src="${escape(e.image)}" alt="${escape(e.title)}" loading="lazy"></a>`:''}<div class="annals-entry-bottom"><span>Filed by ${escape(e.author)}${e.words?' · '+Math.ceil(e.words/220)+' min read':''}${e.updatedAt?' · source updated '+date(e.updatedAt+'T00:00:00Z'):''}</span><a href="${e.kind==='document'?docLink(e.id):escape(e.download)}" ${e.kind==='attachment'?'target="_blank" rel="noopener"':''}>${e.kind==='document'?'Read the writing':'Open '+(e.id==='campaign-image-gallery'?'44-page gallery':'map')} ↗</a></div></article>`;
}
function renderDocument(main,db,id){
 ...
 const contents=record.toc||(headings.length>2?headings.map(b=>({index:b.index,label:b.text})):[]);
 const filed=record.dateKind==='added'?`Added to the archive ${date(record.published)}`:`Filed by ${escape(record.author)} · ${date(record.published)} · ${clock(record.published)} UTC`;
 const provenance=record.updatedAt?`<br>Source updated ${date(record.updatedAt+'T00:00:00Z')}${record.updatedName?' · '+escape(record.updatedName):''}`:'';
